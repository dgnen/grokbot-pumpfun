"""Weight fitting from the log: arithmetic, not advice.

The script answers "what would have happened at other weights" on
trades that already happened, so we check the count: whom the
threshold keeps, whom it loses, and what lands in the candidates.
"""
...
def candidates() -> list[tune.Candidate]:
    return [
        tune.Candidate("good", (0.9, 0.9, 0.9, 0.9), True, +1.0),
        tune.Candidate("bad", (0.4, 0.4, 0.4, 0.4), True, -0.5),
        tune.Candidate("untraded", (0.7, 0.7, 0.7, 0.7), False, None),
    ]
...
def test_threshold_splits_kept_and_lost():
    ...
    assert outcome.passed == 2                 # good and untraded
...
def test_score_rewards_cutting_losers():
    """A threshold that cuts a losing trade must rank higher."""
    weights = (0.25, 0.25, 0.25, 0.25)
...
def test_rank_prefers_less_degenerate_weights():
    """On a tie, a balanced set beats a 1.00 weight on one component."""
...
def test_current_weights_fall_back_to_defaults(tmp_path):
    weights = tune.current_weights(str(tmp_path / "missing.yaml"))
...
def test_empty_log_exits_with_error(tmp_path, monkeypatch, capsys):
    ...
    assert tune.main() == 1
    assert "No scoring records" in capsys.readouterr().out
...
def test_main_runs_on_real_shaped_log(tmp_path, monkeypatch, capsys):
    ...
    assert "Threshold at current weights" in printed
    assert "Top 3" in printed
...
def test_mixed_prompt_versions_are_reported(tmp_path, monkeypatch, capsys):
    ...
    tune.main()
    printed = capsys.readouterr().out
    assert "different prompt versions" in printed
...
def test_single_version_is_not_reported(tmp_path, monkeypatch, capsys):
    ...
    tune.main()
    assert "different prompt" not in capsys.readouterr().out
